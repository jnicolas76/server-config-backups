...
This bundle installs CineMediaVault with optional Movies, TV Shows, Music, Comics, NES, SEGA, DOS, and MAME modules.
...
MUSIC_ROOT="/mnt/nfs-share-movies/Music"
...
Set `MUSIC_ROOT` to the organized music library. Music is intentionally excluded from the regular Movies/TV scan because large music collections take much longer to walk. Use the separate **Scan Music** button when files change. The scan is incremental: unchanged files are identified by path, modification time, and size and do not have their metadata probed again. Automatic startup scans are disabled by default; set `CINEMEDIAVAULT_MUSIC_AUTO_SCAN=1` only if they are explicitly wanted.

The Music page supports searching, seeking, shuffle/repeat, persistent per-user playlists, individual downloads, and playlist ZIP downloads containing tracks, artwork, and `manifest.json` metadata for offline clients. Scan progress remains visible after a browser refresh, and the Back arrow returns to the previous CineMediaVault page.
