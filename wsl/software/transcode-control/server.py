...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/advanced.css":
            return self.send_file(ROOT / "static" / "advanced.css", "text/css; charset=utf-8")
        if path == "/api/session":
        ...
        if path == "/api/dashboard":
            ...
            persisted = {x["key"]: json.loads(x["value"]) for x in settings}
            payload["configuration"] = {**payload.get("configuration", {}), **persisted}
    ...
    def do_POST(self):
        ...
        if path == "/api/commands":
            ...
            allowed = {"move","remove","add","set_profile","save_configuration","start","stop","refresh"}
            if action not in allowed: return self.send_json({"error": "invalid action"}, 400)
            payload = data.get("payload", {})
            with db() as con:
                if action == "save_configuration":
                    section = str(payload.get("section", ""))
                    if section not in {"libraries", "profiles", "rules", "workers"}:
                        return self.send_json({"error": "invalid configuration section"}, 400)
                    con.execute(
                        "INSERT INTO settings(key,value,updated_at) VALUES(?,?,?) "
                        "ON CONFLICT(key) DO UPDATE SET value=excluded.value,updated_at=excluded.updated_at",
                        (section, json.dumps(payload.get("value", {})), now()),
                    )
