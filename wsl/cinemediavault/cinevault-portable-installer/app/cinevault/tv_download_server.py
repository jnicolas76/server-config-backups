...
def tmdb_tv_credits(tv_id: int) -> list[str]:
    payload = tmdb_request_json(f"/tv/{tv_id}/credits", {})
    return [person.get("name", "") for person in payload.get("cast", [])[:10] if person.get("name")]
...
class Handler(BaseHTTPRequestHandler):
        ...
        for index, season in enumerate(seasons):
            ...
            for episode in season.episodes:
                ...
                episodes.append(
                    ...
                    f"<a class='episode-download' href='/download/episode/{episode.id}'>Download</a>"
                    f"<a class='episode-more' href='/player/tv/{episode.id}'>More</a>"
                    f"{f'<a class=\"episode-admin\" href=\"/admin/media/tv/{episode.id}\">Admin</a>' if is_admin else ''}"
                    f"</span>"
                    f"</li>"
                )
            ...
            season_blocks.append(
                ...
                f"<summary><span>{html.escape(season.label)} <small>{len(season.episodes)} episodes - {human_size(season_size)}</small></span><a class='season-download' href='/download/season/{urllib.parse.quote(season.key)}'>Season ZIP</a></summary>"
    ...
    def download_season(self, key: str):
        ...
        filename = urllib.parse.quote(f"{season.label}.zip")
        self.send_response(200)
...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box; }
    ...
    .episode-actions { display:flex; gap:7px; justify-content:flex-end; }
