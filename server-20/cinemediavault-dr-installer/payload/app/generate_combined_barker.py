...
def choose_items(items, count, seed, not_before=0):
    eligible = [x for x in items if int(x.get("airtime") or 0) >= int(not_before)]
    movies = [x for x in eligible if x.get("kind") == "movie" and Path(x.get("path", "")).is_file()]
    television = [x for x in eligible if x.get("kind") == "tv" and Path(x.get("path", "")).is_file()]
...
def main():
    ...
    # Do not advertise anything airing during this reel's six-hour broadcast
    # window; every promotion must still be genuinely upcoming when the reel ends.
    items = choose_items(manifest.get("items", []), count, slot, not_before=slot + 6 * 3600)
    if not items:
