...
def episode_card_subtitle(episodes: list[Episode]) -> str:
    ...
    return f"{season_label} - {episode_label}" if episode_label else season_label
...
class Handler(BaseHTTPRequestHandler):
    ...
    def show_detail(self, show_id: str):
        ...
        subtitle_summary = globals().get("subtitle_summary_for_path", lambda path: "None found")(first_episode.path) if first_episode else "None found"
        season_blocks = []
    ...
    def download_season(self, key: str):
        ...
        session_key = f"season-zip:{secrets.token_urlsafe(8)}"
        cinevault_usage.session_start(session_key, user=user, service="tv", delivery="download",
                                       media_type="season", title=zip_name, transcoding=False,
                                       client=self.headers.get("User-Agent", ""), kind="hold")
        counted = cinevault_usage.CountingWriter(
            self.wfile, lambda n: cinevault_usage.record_bytes(user, "tv", "download", n, session_key=session_key)
        )
        try:
            with zipfile.ZipFile(counted, "w", compression=zipfile.ZIP_STORED) as archive:
    ...
    def serve_file(self, path: Path, disposition: str = "attachment", service: str = "tv"):
        ...
        self.end_headers()
        user = self.current_user()
        delivery = "download" if disposition == "attachment" else "direct"
        session_key = f"tv-file:{secrets.token_urlsafe(8)}"
        cinevault_usage.session_start(session_key, user=user, service=service, delivery=delivery,
        ...
        try:
            with path.open("rb") as handle:
                ...
                while remaining:
                    ...
                    remaining -= len(chunk)
                    cinevault_usage.record_bytes(user, service, delivery, len(chunk), session_key=session_key)
        finally:
            cinevault_usage.session_end(session_key)
...
<body>
  ...
  <header>
    <div class="top-row">
      ...
      <div class="header-actions"><a class="home-link" href="/">Home</a><a class="home-link" href="/genres/tv">Genres</a><button class="playback-toggle" id="playbackModeButton" type="button" data-mode="direct">Direct</button><button class="search-button" id="searchToggle" type="button" aria-label="Search" title="Search">⌕</button></div>
    </div>
...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box; }
    ...
    .detail-page { position:relative; min-height:100vh; min-height:100dvh; overflow:hidden; padding:22px clamp(22px,4vw,56px) calc(22px + env(safe-area-inset-bottom)); }
    ...
    .episode-actions { grid-column:1 / -1; }
    ...
    .season li { display:grid; grid-template-columns:minmax(300px,1fr) auto; gap:10px 14px; align-items:center; padding:12px 0; border-top:1px solid rgba(255,255,255,.08); color:#eef5ff; }
