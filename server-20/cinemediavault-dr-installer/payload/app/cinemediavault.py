...
TRANSCODE_CONTROL_DB = Path(os.environ.get("TRANSCODE_CONTROL_DB", "/home/jnicolas/transcode-control/data/transcode-control.db")).resolve()
POSTER_ROTATION_SECONDS = max(60, int(os.environ.get("CINEVAULT_POSTER_ROTATION_SECONDS", "21600")))
POSTER_ROTATION_CACHE_FILE = Path(os.environ.get("CINEVAULT_POSTER_ROTATION_CACHE_FILE", str(CINEVAULT_DB.parent / "poster-rotation-cache.json"))).resolve()
...
DEFAULT_MODULES = [
    {"id": "whisper", "name": "Whisper Subtitles", "logo": "CC", "enabled": False, "protocol": "http", "port": 0, "url": "", "start_script": "WSL control agent", "pid_file": "", "stop_pattern": "", "remote_control": "whisper"},
...
def module_running(module: dict) -> bool:
    if module.get("remote_control") == "whisper":
        return whisper_remote_status().get("running", False)
...
def start_module(module: dict) -> tuple[bool, str]:
    if module.get("remote_control") == "whisper":
        return queue_whisper_command("whisper_start")
...
def stop_module(module: dict) -> tuple[bool, str]:
    if module.get("remote_control") == "whisper":
        return queue_whisper_command("whisper_stop")
    stopped = False
    pid_file = Path(str(module.get("pid_file") or ""))
    try:
        if pid_file.is_file():
...
def whisper_remote_status() -> dict:
    """Read the latest WSL-reported Whisper state from the local control DB."""
    try:
        uri = f"file:{TRANSCODE_CONTROL_DB}?mode=ro"
        conn = sqlite3.connect(uri, uri=True, timeout=2)
        try:
            row = conn.execute("SELECT created_at,payload FROM snapshots ORDER BY id DESC LIMIT 1").fetchone()
        finally:
            conn.close()
        if not row:
            return {"running": False, "available": False, "detail": "No WSL status received"}
        payload = json.loads(row[1])
        state = dict(payload.get("whisper") or {})
        state["available"] = bool(state)
        state["reported_at"] = row[0]
        return state
    except Exception as exc:
        return {"running": False, "available": False, "detail": str(exc)}


def queue_whisper_command(action: str) -> tuple[bool, str]:
    """Queue one whitelisted operation for the authenticated WSL polling agent."""
    if action not in {"whisper_start", "whisper_stop"}:
        return False, "Unsupported Whisper action."
    try:
        conn = sqlite3.connect(TRANSCODE_CONTROL_DB, timeout=5)
        try:
            pending = conn.execute(
                "SELECT id FROM commands WHERE action=? AND status IN ('pending','sent') ORDER BY id DESC LIMIT 1",
                (action,),
            ).fetchone()
            if not pending:
                conn.execute(
                    "INSERT INTO commands(created_at,action,payload,status) VALUES(?,?,?,'pending')",
                    (datetime.datetime.now(datetime.timezone.utc).isoformat(timespec="seconds"), action, "{}"),
                )
                conn.commit()
        finally:
            conn.close()
        verb = "start" if action.endswith("start") else "stop"
        return True, f"Whisper {verb} requested. Status updates automatically within about 10 seconds."
    except Exception as exc:
        return False, f"Could not contact the WSL control queue: {exc}"


class MultipartField:
    """Minimal stand-in for the pieces of cgi.FieldStorage this app used.

    cgi.FieldStorage is removed in Python 3.13 (PEP 594). The only user of
    it in this file was the admin module-logo upload, which only ever needs
    one plain text field ("module_id") and one file field ("logo_file").
    This class plus parse_multipart_form() replace that narrow usage with
    the stdlib email package, which stays available in 3.13+.
    """
...
def parse_multipart_form(content_type: str, body: bytes) -> dict:
    ...
    Returns a dict mapping field name -> str (text fields) or
    MultipartField (file fields), matching the subset of cgi.FieldStorage
    behavior this app relies on.
    """
    match = re.search(r'boundary="?([^";]+)"?', content_type or "", re.IGNORECASE)
    if not match:
        return {}
    header_bytes = f"Content-Type: {content_type}\r\nMIME-Version: 1.0\r\n\r\n".encode("utf-8")
    message = email.message_from_bytes(header_bytes + body, policy=email.policy.compat32)
    fields: dict = {}
    ...
    for part in message.get_payload():
        ...
        if filename:
            fields[name] = MultipartField(filename=filename, data=payload)
        else:
...
def ensure_auth_schema() -> None:
    ...
    try:
        ...
        now = auth_now()
        # CineMediaVault installer patch: the installer supplies the first
        # administrator through a root-owned
        # bootstrap file. A distributable build must never contain a known
        # username, password, or site-specific account.
        bootstrap_path = os.environ.get("CINEVAULT_ADMIN_BOOTSTRAP", "")
        bootstrap = None
        if bootstrap_path:
            try:
                with open(bootstrap_path, "r", encoding="utf-8") as bootstrap_file:
                    candidate = json.load(bootstrap_file)
                if (isinstance(candidate, dict) and candidate.get("username")
                        and str(candidate.get("password_hash", "")).startswith("pbkdf2_sha256$")):
                    bootstrap = candidate
        ...
        if bootstrap:
            conn.execute(
                ...
                """,
                (
                    str(bootstrap.get("username")),
                    str(bootstrap.get("full_name") or ""),
                    str(bootstrap.get("email") or ""),
                    str(bootstrap.get("password_hash")),
                    now,
                    now,
                ),
            )
...
def refresh_hdhr_guide(force: bool = False) -> dict:
    with HDHR_GUIDE_LOCK:
        ...
        # Extend past the ~2-day HDHomeRun horizon with the verified iptv-org/epg feed.
        # This never removes or rewrites HDHomeRun data and is a no-op on any failure.
        try:
            payload["extended"] = epg_extend.merge_into_payload(payload)
        except Exception as exc:
            payload["extended"] = {"ok": False, "error": f"merge raised: {exc}"}
        payload["coverage"] = epg_extend.coverage(payload)
        ...
        return {"ok": True, "cached": False, "channels": len(programmes),
                "programmes": payload["programme_count"], "hdhomerun_programmes": count,
                "extended": payload.get("extended", {}), "coverage": payload.get("coverage", {})}


HDHR_GUIDE_SNAPSHOT_CACHE: dict = {"key": None, "data": None}


def hdhr_guide_snapshot() -> dict:
    try:
        stat = HDHR_GUIDE_CACHE_FILE.stat()
        key = (stat.st_mtime_ns, stat.st_size)
        cached = HDHR_GUIDE_SNAPSHOT_CACHE
        if cached["key"] == key and cached["data"] is not None:
            return cached["data"]
        data = json.loads(HDHR_GUIDE_CACHE_FILE.read_text(encoding="utf-8"))
        cached["key"] = key
        cached["data"] = data
        return data
    except Exception:
  ...
  <style>
    ...
    * { box-sizing:border-box; }
    ...
    @media (max-width:640px) {
      ...
      video { display:block; width:100%; max-height:70vh; border-radius:6px; }
    }
  </style>
</head>
<body>
...
def _build_home_backdrop_html(movie_items=None, tv_shows=None, limit: int = 70) -> str:
    posters: list[str] = []
...
def _build_login_backdrop_html(limit: int = 70) -> str:
    posters: list[str] = []
...
def cached_poster_backdrop(cache_key: str, builder) -> str:
    """Keep a selected poster set stable across requests and service restarts."""
    now = time.time()
    with POSTER_ROTATION_LOCK:
        try:
            cache = json.loads(POSTER_ROTATION_CACHE_FILE.read_text(encoding="utf-8"))
            if not isinstance(cache, dict):
                cache = {}
        except (OSError, json.JSONDecodeError):
            cache = {}
        entry = cache.get(cache_key) if isinstance(cache.get(cache_key), dict) else {}
        if float(entry.get("expires_at") or 0) > now and isinstance(entry.get("html"), str):
            return entry["html"]
        rendered = builder()
        cache[cache_key] = {"created_at": now, "expires_at": now + POSTER_ROTATION_SECONDS, "html": rendered}
        POSTER_ROTATION_CACHE_FILE.parent.mkdir(parents=True, exist_ok=True)
        temporary = POSTER_ROTATION_CACHE_FILE.with_suffix(POSTER_ROTATION_CACHE_FILE.suffix + ".tmp")
        temporary.write_text(json.dumps(cache, separators=(",", ":")), encoding="utf-8")
        temporary.replace(POSTER_ROTATION_CACHE_FILE)
        return rendered


def home_backdrop_html(movie_items=None, tv_shows=None, limit: int = 70) -> str:
    if movie_items is not None or tv_shows is not None or limit != 70:
        return _build_home_backdrop_html(movie_items, tv_shows, limit)
    return cached_poster_backdrop("home", lambda: _build_home_backdrop_html(limit=limit))


def random_login_backdrop_html(limit: int = 70) -> str:
    if limit != 70:
        return _build_login_backdrop_html(limit)
    return cached_poster_backdrop("login", lambda: _build_login_backdrop_html(limit))
...
def movie_for_stale_saved_id(user, numeric_id: str):
    """Resolve a scan-order ID through this user's stable saved movie key."""
    try:
        requested_id = int(numeric_id)
    except (TypeError, ValueError):
        return None
    conn = db_connect()
    try:
        rows = conn.execute(
            """SELECT media_key, stamp FROM (
                 SELECT media_key, updated_at AS stamp FROM user_media_state
                 WHERE user_id=? AND media_type='movie' AND media_id=?
                 UNION ALL
                 SELECT media_key, last_played_at AS stamp FROM user_play_history
                 WHERE user_id=? AND media_type='movie' AND media_id=?
               ) ORDER BY stamp DESC""",
            (int(user["id"]), requested_id, int(user["id"]), requested_id),
        ).fetchall()
    finally:
        conn.close()
    resolved_by_key = {}
    for row in rows:
        ...
        if item and int(item.id) != requested_id:
...
const wall=document.getElementById('wall'),tiles=document.getElementById('tiles'),drawer=document.getElementById('drawer'),search=document.getElementById('wallSearch'),results=document.getElementById('results');let slots=[],active=1,timer,hlsControllers=[];
...
<main>{note}<h1>Modules</h1><p class="muted">Show or hide external libraries on the CineMediaVault home page, change their destination URLs, and start or stop the backing services.</p><p><a href="/admin/vchannels">Manage 24×7 Virtual Channel Schedules</a></p><div class="module-grid">{''.join(rows)}</div></main></body></html>""")
    ...
    def admin_module_logo_submit(self, user):
        try:
            ...
            module_id = str(form.get("module_id") or "").strip()
            file_item = form.get("logo_file")
        except Exception as exc:
...
<body><section class="panel"><h1>Deleted</h1><p>{html.escape(deleted_path)}</p><p><a href="/{'movies' if kind == 'movie' else 'tv'}">Back to library</a></p></section></body></html>""")
    ...
    def api_hdhr_devices(self, user):
        ...
        return self.json_response({"ok": True, "devices": devices, "channels": channels,
            ...
            "guide_extended": guide.get("extended", {}), "guide_coverage": guide.get("coverage", {})})
