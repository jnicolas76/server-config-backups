...
def generate_horizon(movie_app, tv_app, horizon_days=HORIZON_DAYS):
    ...
    try:
        ...
        c.close()
    print(f"Virtual schedule horizon generated through {horizon_until} in {time.monotonic() - build_started:.1f} seconds", flush=True)
...
<section class="card"><h2>Generate Barker Preview</h2><p>Render the next six-hour, 37-second-segment promotional reel in the background. The current validated reel remains live until the new one is ready.</p><form method="post" action="/admin/vchannels" class="single-run"><button name="action" value="generate_barker"{disabled}>Generate Barker Preview</button></form></section>
...
def generate_barker_admin(movie_app, tv_app):
    write_combined_barker_manifest(movie_app, tv_app, limit=1000)
    log_path = COMBINED_BARKER_DIR / "admin-generation.log"
    ...
    if path == "/admin/vchannels":
        ...
        try:
            if action == "regenerate":
                started, status = start_admin_operation("Schedule regeneration", lambda: rebuild_schedules(movie_app, tv_app, randomize=True))
            ...
            if action == "rebuild_tv":
                ...
                started, status = start_admin_operation("TV schedule regeneration", lambda: rebuild_tv_schedules(movie_app, tv_app, randomize=True))
                return admin_page(handler, status)
