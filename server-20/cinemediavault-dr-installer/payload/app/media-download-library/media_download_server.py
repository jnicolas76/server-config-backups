...
class Handler(BaseHTTPRequestHandler):
    ...
    def serve_media(self, item_id: str, disposition: str, head_only: bool = False):
        ...
        session_key = f"movie-dl:{secrets.token_urlsafe(8)}"
        cinevault_usage.session_start(session_key, user=user, service="movie", delivery=delivery,
                                       media_type="movie", media_id=item.id, title=item.title,
                                       transcoding=False, client=self.headers.get("User-Agent", ""), kind="hold")
        try:
            with item.path.open("rb") as handle:
                ...
                while remaining:
                    ...
                    remaining -= len(chunk)
                    cinevault_usage.record_bytes(user, "movie", delivery, len(chunk), session_key=session_key)
        finally:
...
<body>
  ...
  <header>
    <div class="top-row">
      ...
      <div class="header-actions"><a class="home-link" href="/">Home</a><a class="home-link" href="/genres/movies">Genres</a><button class="playback-toggle" id="playbackModeButton" type="button" data-mode="direct">Direct</button><button class="search-button" id="searchToggle" type="button" aria-label="Search" title="Search">⌕</button></div>
    </div>
