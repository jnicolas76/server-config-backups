...
CINEVAULT_LEGACY_ENABLED="${CINEVAULT_LEGACY_ENABLED:-0}"
...
# Port 8093 is retained only as an opt-in rollback path. The former lab on
# HTTPS port 5000 is now the canonical CineVault application.
if [[ "$CINEVAULT_LEGACY_ENABLED" == "1" ]]; then
  start_cmd "CineVault Legacy" "$CINEVAULT_PORT" "$BASE/cinevault-8093.pid" "$BASE/cinevault-8093.log" \
    env CINEVAULT_MUSIC_ROOT="/media/jnicolas/Expansion/Music" \
        CINEVAULT_MUSIC_DB="$BASE/cinevault-data/music.db" \
        CINEVAULT_MUSIC_ART_CACHE="$BASE/cinevault-data/music-art" \
        python3 "$BASE/cinevault-watch-8093.py"
else
  log "CineVault legacy port 8093 disabled; canonical service is HTTPS port 5000"
fi

# Canonical CineVault instance, promoted from the former lab.
start_script_bg "CineVault" "$CINEVAULT_LAB_PORT" "$BASE/cinemediavault-lab/start_lab_5000.sh" \
