...
SCAN_PROGRESS_FILE = Path(os.environ.get("SCAN_PROGRESS_FILE", "/home/jnicolas/.cinemediavault-scan-progress-8093.json")).resolve()
...
def rebuild_actor_index() -> dict:
    ...
    for item in movie_app.movie_index.items:
        ...
        for name in actor_names_for(metadata):
            rows.append((name, actor_name_norm(name), "movie", int(item.id), title, str(metadata.get("year") or movie_app.human_size(item.size)), str(movie_app.poster_url_for(item) or ""), f"/movie/{item.id}"))
    for show in tv_app.tv_index.shows:
        metadata = tv_app.metadata_for(show)
        title = str(metadata.get("title") or show.title)
        for name in actor_names_for(metadata):
            rows.append((name, actor_name_norm(name), "tv", int(show.id), title, f"{show.count} episodes", str(tv_app.poster_url_for(show) or ""), f"/tv/show/{show.id}"))
    conn = db_connect()
    try:
        ...
        for name, norm, media_type, media_id, title, subtitle, poster, href in rows:
            ...
            conn.execute("INSERT OR IGNORE INTO actors(name,name_norm) VALUES(?,?)", (name, norm))
            actor_id = conn.execute("SELECT id FROM actors WHERE name_norm=?", (norm,)).fetchone()["id"]
        ...
        conn.commit()
        return {"actors": conn.execute("SELECT COUNT(*) FROM actors").fetchone()[0], "links": conn.execute("SELECT COUNT(*) FROM actor_media").fetchone()[0]}
    finally: conn.close()
...
def actor_search_results(query: str, limit: int = 20) -> list[dict]:
    ...
    try:
        ...
        return [{"kind":"Actor","title":row["name"],"subtitle":f"{row['media_count']} library title(s)","poster":row["poster"],"href":f"/actor?name={urllib.parse.quote(row['name'])}"} for row in rows]
    finally: conn.close()
...
def actor_media_results(name: str) -> list[dict]:
    ...
    try:
        ...
        return [{"kind":"Movie" if row["media_type"]=="movie" else "TV Show","title":row["title"],"subtitle":row["subtitle"],"poster":row["poster"],"href":row["href"]} for row in rows]
    finally: conn.close()
...
def comic_collection_count() -> int:
    roots = [
        COMIC_LIBRARY_ROOT / "collections",
        COMIC_LIBRARY_ROOT,
        Path("/media/jnicolas/Expansion/Movies/comic-library/collections"),
        Path("/media/jnicolas/Expansion/Movies/Comics"),
    ]
    for root in roots:
        count = count_dirs(root)
        if count:
            return count
    return 0


def comic_file_stats() -> tuple[int, int]:
    roots = [
        COMIC_LIBRARY_ROOT,
        Path("/media/jnicolas/Expansion/Movies/comic-library"),
        Path("/media/jnicolas/Expansion/Movies/Comics"),
    ]
    extensions = {".cbr", ".cbz", ".pdf", ".jpg", ".jpeg", ".png", ".webp", ".html"}
    for root in roots:
        count, size = count_files(root, extensions, max_seconds=2.0, max_files=50000)
        if count:
            return count, size
    return 0, 0
...
def homepage_status_payload(handler=None) -> dict:
    ...
    comic_files, comic_size = comic_file_stats()
    ...
    return {
        "ok": True,
        "name": "CineMediaVault",
        "url": f"http://{handler.request_hostname()}:8093" if handler else "",
        "movies": int(stats["movies"]["assets"]),
        ...
        "downloads": len(TRANSCODES),
        "download_jobs": len(TRANSCODES),
...
<main>
  ...
  <div class="layout">
    ...
    def actor_page(self):
        ...
        cards = "".join(search_result_card(item) for item in results) or "<p class='muted'>No matching library titles found.</p>"
