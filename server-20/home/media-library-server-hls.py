...
def ensure_auth_schema() -> None:
    ...
    try:
        ...
        conn.execute(
            """
            INSERT INTO users(username, full_name, email, password_hash, is_admin, active, created_at, updated_at)
            VALUES('jnicolas', 'Jonathan Nicolas', 'jonathan.nicolas@gmail.com', ?, 1, 1, ?, ?)
...
<body>
  ...
  <main>
      <div>
        ...
        <div class="message" id="message"><strong>Starting stream.</strong> If this is the first play, CineVault is creating HLS segments now.</div>
      </div>
    </main>
  </div>
...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box; }
    ...
    .poster-cloud { position:absolute; left:-90px; right:-90px; top:78px; width:auto; display:grid; grid-template-columns:repeat(12, minmax(72px, 1fr)); gap:12px; transform:rotate(10deg); z-index:1; opacity:.86; }
    ...
    @media (max-width:1040px){
      ...
      .poster-cloud { opacity:.52; left:-190px; right:-220px; top:110px; grid-template-columns:repeat(10, minmax(68px, 1fr)); }
    ...
    @media (max-width:620px){
      ...
      .poster-cloud { left:-210px; right:-280px; top:72px; gap:9px; opacity:.44; grid-template-columns:repeat(8, minmax(66px, 1fr)); transform:rotate(10deg) scale(.98); }
...
<body>
  <section class="hero">
    <nav class="nav">
      <div class="brand-wrap"><div class="brand">Cine<span>Vault</span></div><div class="brand-credit">Brought to you by Jonathan Nicolas</div></div>
...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box; }
    ...
    .home-backdrop { position:fixed; inset:-80px -90px auto -90px; height:calc(100vh + 160px); z-index:0; display:grid; grid-template-columns:repeat(14, minmax(72px, 1fr)); gap:13px; transform:rotate(-10deg) translateY(-34px); opacity:.38; pointer-events:none; overflow:hidden; filter:saturate(1.12) contrast(1.04); }
    ...
    .scan-button { min-height:38px; display:inline-flex; align-items:center; padding:0 14px; border-radius:999px; background:rgba(245,183,63,.18); border:1px solid rgba(245,183,63,.42); color:#fff; font-weight:900; cursor:pointer; }
    .scan-button.running { opacity:.7; cursor:wait; }
    .avatar { width:29px; height:29px; display:grid; place-items:center; border-radius:999px; background:#75a9d9; color:#fff; font-weight:900; font-size:14px; }
    ...
    .tab { flex:0 0 auto; display:inline-flex; align-items:center; justify-content:center; min-height:42px; padding:0 18px; border-radius:14px; border:1px solid rgba(255,255,255,.18); color:#eee; text-decoration:none; font-size:17px; font-weight:800; background:rgba(255,255,255,.06); }
    ...
    @media (hover: hover), (min-width: 900px) {
      ...
      .home-search-panel input { height:30px; border-radius:9px; font-size:10px; }
      .scan-button { min-height:24px; padding:0 8px; font-size:7px; }
      .icon { width:22px; height:22px; font-size:10px; }
      .avatar { width:16px; height:16px; font-size:6px; }
      .tabs { gap:8px; padding:0 18px 16px; }
      .tab { flex:0 0 auto; min-height:24px; padding:0 10px; border-radius:9px; font-size:7px; }
      ...
      .sub { font-size:6px; margin-top:2px; }
      .rail { gap:12px; padding:0 18px 5px; overflow-x:auto; overflow-y:hidden; }
      .card { flex:0 0 118px; width:118px; }
      .poster { border-radius:6px; box-shadow:0 8px 18px rgba(0,0,0,.38); }
      .progress { left:6px; right:6px; bottom:6px; height:4px; }
      .card-title { margin-top:6px; font-size:6px; line-height:1.1; }
      .card-meta { margin-top:2px; font-size:5px; }
      .libraries { gap:12px; padding:0 18px 4px; overflow-x:auto; overflow-y:hidden; }
      .library { flex:0 0 174px; width:174px; min-height:42px; gap:8px; padding:0 12px; border-radius:19px; }
      .library-icon { width:18px; height:18px; flex-basis:18px; }
      .library-title { font-size:9px; }
      .library-sub { font-size:6px; }
      main { padding-bottom:42px; }
    }
    @media (max-width: 560px) and (hover: none) and (pointer: coarse) {
      body { padding-bottom:78px; }
      .bottom-nav { display:grid; }
      .home-backdrop { height:360px; inset:-42px -160px auto -140px; grid-template-columns:repeat(10, minmax(72px,1fr)); opacity:.36; }
      ...
      .tab { min-height:38px; padding:0 15px; font-size:16px; }
      h2 { font-size:25px; }
...
<body>
  ...
  <header>
    <div class="brand">Cine<span>Vault</span></div>
    <div class="top-actions"><button class="scan-button" id="scanButton">Scan</button><button class="icon" id="homeSearchToggle" type="button" aria-label="Search" title="Search">⌕</button><button class="icon cast-button" id="castButton" type="button" aria-label="Cast" title="Cast"><svg viewBox="0 0 32 32" aria-hidden="true"><path d="M5 10V7.5C5 6.1 6.1 5 7.5 5h17C25.9 5 27 6.1 27 7.5v17c0 1.4-1.1 2.5-2.5 2.5H22" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round"/><path d="M5 21c3.3 0 6 2.7 6 6M5 15c6.6 0 12 5.4 12 12M5 27h.1" fill="none" stroke="currentColor" stroke-width="2.8" stroke-linecap="round"/></svg></button><div class="avatar">J</div></div>
  </header>
  <div class="home-search-panel" id="homeSearchPanel"><input id="homeSearch" type="search" placeholder="Search movies, shows, actors, or genres"></div>
  <main>
    <nav class="tabs"><a class="tab active" href="/">Home</a><a class="tab" href="/movies">Movies</a><a class="tab" href="/tv">TV Shows</a><a class="tab external service-link" data-protocol="http:" data-port="8110" href="#">Comics</a><a class="tab external service-link" data-protocol="https:" data-port="8092" href="#"><span class="game-logo logo-nes">NES</span></a><a class="tab external service-link" data-protocol="https:" data-port="8094" href="#"><span class="game-logo logo-sega">SEGA</span></a><a class="tab external service-link" data-protocol="http:" data-port="8091" href="#"><span class="game-logo logo-dos">DOS</span></a><a class="tab external service-link" data-protocol="https:" data-port="8101" href="#"><span class="game-logo logo-mame">MAME</span></a></nav>
    ...
    <section class="section">
      <div class="section-head"><div><h2>Recently Added in Movies</h2><div class="sub">{{SERVER_NAME}}</div></div><a href="/movies">›</a></div>
      <div class="rail">{{RECENT_MOVIES}}</div>
    </section>
    <section class="section">
      <div class="section-head"><div><h2>Recently Added in TV Shows</h2><div class="sub">{{SERVER_NAME}}</div></div><a href="/tv">›</a></div>
  ...
  <script>
    ...
    if (homeSearchToggle && homeSearchPanel && homeSearch) {
      ...
      homeSearch.addEventListener("keydown", event => {
        if (event.key === "Enter" && homeSearch.value.trim()) {
          location.href = `/movies?q=${encodeURIComponent(homeSearch.value.trim())}`;
        }
    ...
    function mediaTitleForCast() {
      ...
      return title ? title.textContent.trim() : "CineVault";
    }
    ...
    function ensureCastPanel() {
      ...
      if (!panel) {
        ...
        panel.innerHTML = `<div class="cast-sheet"><button class="cast-close" type="button" aria-label="Close">×</button><h2>Connect To</h2><div class="cast-list" id="castDeviceList"></div><p class="cast-note" id="castNote"></p></div>`;
    ...
    async function openCastPicker() {
      setCastPanel("Searching local network from the CineVault server...");
      try {
    ...
    document.querySelectorAll(".service-link").forEach(link => {
      const protocol = link.dataset.protocol || location.protocol;
      const port = link.dataset.port;
      link.href = `${protocol}//${location.hostname}:${port}/`;
    });
    function esc(value) {
...
<body>
  <div class="page">
    <header><a class="back" href="{{BACK}}">‹</a><div class="text"><div class="title">{{TITLE}}</div><div class="sub">{{SUBTITLE}}</div></div></header>
...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box; }
    ...
    .restart::before { content:"↻"; font-size:23px; line-height:1; transform:translateX(-2px); }
    .restart::after { content:"▶"; position:absolute; font-size:11px; line-height:1; transform:translate(5px,1px); }
    ...
    .overlay-close { display:grid; place-items:center; width:46px; height:46px; border-radius:999px; color:#fff; text-decoration:none; border:1px solid rgba(255,255,255,.14); background:rgba(0,0,0,.28); font-size:34px; line-height:1; }
  ...
  <main class="watch-page"{{BACKGROUND_STYLE}}>
    ...
    <section class="player-shell{{PLAYER_OPEN_CLASS}}{{PLAYER_FULLSCREEN_CLASS}}" id="playerShell">
      <div class="player-head">
        <a class="circle" href="{{BACK}}" aria-label="Back">‹</a>
        <div class="player-title"><strong>{{EPISODE_OR_TITLE}}</strong><span>{{SUBTITLE}}</span></div>
        <div class="episode-nav">{{PLAYER_NAV}}</div>
      </div>
      <div class="video-wrap">
        <video id="player" playsinline preload="metadata"{{VIDEO_AUTOPLAY}} data-source="{{SOURCE}}"></video>
        <div class="player-overlay visible" id="playerOverlay">
          <div class="overlay-top">
            <div class="overlay-title"><strong>{{EPISODE_OR_TITLE}}</strong><span>{{SUBTITLE}}</span></div>
            <a class="overlay-close" href="{{BACK}}" aria-label="Close player">×</a>
          </div>
          <div></div>
        ...
        <div class="up-next" id="upNext">
          <div class="up-next-grid">
            ...
            <div>
              <div class="up-next-kicker">Up Next • <span id="upNextCount">10</span></div>
...
def home_backdrop_html(movie_items, tv_shows, limit: int = 42) -> str:
    posters: list[str] = []
...
def random_login_backdrop_html(limit: int = 50) -> str:
    posters: list[str] = []
...
def continue_item_for_episode(episode) -> dict:
    ...
    still_path = str(episode_meta.get("still_path") or "")
    if still_path and still_path.startswith("/"):
        still_path = f"https://image.tmdb.org/t/p/w780{still_path}"
...
def action_link(icon: str, label: str, href: str, attrs: str = "") -> str:
    ...
    return f"<a class='action{extra_class}' href='{html.escape(href)}'{attrs}><span>{html.escape(icon)}</span>{html.escape(label)}</a>"
...
def direct_player_context(kind: str, item_id: str, is_admin: bool = False, playback_mode: str | None = None) -> dict:
    playback_mode = (playback_mode or CINEVAULT_PLAYBACK_MODE or "direct").lower()
...
class CombinedHandler(BaseHTTPRequestHandler):
    ...
    def current_user(self):
        ...
        try:
            row = conn.execute(
                """
                SELECT users.id, users.username, users.full_name, users.email, users.is_admin, users.active
                FROM user_sessions
...
.backdrop {{ position:fixed; inset:-10vh -18vw; z-index:0; display:grid; grid-template-columns:repeat(10,minmax(76px,1fr)); gap:12px; transform:rotate(9deg) scale(1.08); opacity:.66; filter:saturate(1.08) contrast(1.02); }}
...
.brand h1 {{ margin:0; font-size:clamp(42px,8vw,76px); line-height:.92; letter-spacing:0; font-weight:950; }} .brand span {{ color:var(--lime); }}
...
@media (max-width:700px) {{ .page {{ padding:30px; }} .backdrop {{ grid-template-columns:repeat(5,minmax(88px,1fr)); gap:10px; inset:-6vh -62vw; }} .content {{ padding-bottom:42px; }} }}
</style></head><body>
<div class="backdrop">{backdrop}</div><div class="veil"></div>
<main class="page">
  <section class="topbar"><div class="brand"><h1>Cine<span>Vault</span></h1></div><button class="signin-open" type="button" id="openLogin">Sign In</button></section>
...
<div class="login-modal{' open' if error else ''}" id="loginModal">
  <form class="login" method="post" action="/login">
      <div class="login-title"><span>Sign in</span><button class="close" type="button" id="closeLogin" aria-label="Close">×</button></div>{error_html}
...
modal.addEventListener("click",event=>{{if(event.target===modal) modal.classList.remove("open");}});
    ...
    def admin_users_page(self, user, message: str = ""):
        conn = db_connect()
        try:
            rows = conn.execute("SELECT id, username, full_name, email, is_admin, active, created_at, last_login_at FROM users ORDER BY username").fetchall()
        finally:
            conn.close()
        user_rows = []
        for row in rows:
            delete = "" if row["username"] == user["username"] else f"""
            ...
            user_rows.append(f"""<tr><td>{html.escape(row['username'])}</td><td>{html.escape(row['full_name'] or '')}</td><td>{html.escape(row['email'] or '')}</td><td>{'Admin' if row['is_admin'] else 'User'}</td><td>{'Active' if row['active'] else 'Disabled'}</td><td>{html.escape(row['last_login_at'] or '')}</td><td>{actions}</td></tr>""")
...
<title>CineVault Users</title><style>
...
.danger {{ background:#62212b; color:#ffdfe4; }} .table-wrap {{ width:100%; overflow-x:auto; }} table {{ width:100%; min-width:920px; border-collapse:collapse; }} th,td {{ padding:10px; border-bottom:1px solid var(--line); text-align:left; vertical-align:middle; }} .actions {{ display:flex; gap:10px; align-items:center; flex-wrap:wrap; }} .actions form {{ margin:0; }} .password-form {{ display:flex; gap:8px; align-items:center; }} .password-form input {{ width:170px; min-height:36px; }} .note {{ padding:10px 12px; border-radius:10px; background:#15351f; margin-bottom:14px; }}
...
</style></head><body><header><strong>CineVault User Management</strong><nav><a href="/">Home</a> · <a href="/logout">Logout</a></nav></header><main>{note}
...
<section class="panel users-panel"><h2>Users</h2><div class="table-wrap"><table><thead><tr><th>Username</th><th>Name</th><th>Email</th><th>Role</th><th>Status</th><th>Last login</th><th>Actions</th></tr></thead><tbody>{''.join(user_rows)}</tbody></table></div></section>
        ...
        try:
            ...
            if action == "delete":
                ...
                if user_id == int(user["id"]):
                    return self.admin_users_page(user, "You cannot delete your own admin account.")
                conn.execute("DELETE FROM users WHERE id=?", (user_id,))
                conn.commit()
                return self.admin_users_page(user, "User deleted.")
    ...
    def dispatch(self, head: bool = False):
        ...
        if path.startswith("/play/episode/"):
            return tv_app.Handler.play_episode(self, path.rsplit("/", 1)[-1])
        ...
        if path.startswith("/play/"):
            return movie_app.Handler.play(self, path.rsplit("/", 1)[-1], head_only=head)
    ...
    def landing(self):
        ...
        avatar = f'<a class="avatar" href="/admin/users" title="User management">{html.escape(initial)}</a>' if user and user["is_admin"] else f'<div class="avatar">{html.escape(initial)}</div>'
        ...
        body = (
            ...
            .replace("{{HOME_BACKDROP}}", home_backdrop_html(recent_movies, recent_shows))
    ...
    def admin_media_page(self, user, kind: str, item_id: str, message: str = ""):
        ...
        return self.render_html(f"""<!doctype html>
<html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>CineVault Admin - {html.escape(item['title'])}</title>
<style>
...
.menu {{ display:grid; gap:18px; margin:8px 0 28px; }} .menu div {{ font-size:22px; font-weight:750; color:#f1f4f8; }}
...
<header><a class="back" href="{html.escape(item['back'])}">‹</a><h1>{html.escape(item['title'])}</h1></header>
<main>{note}
<section class="menu">
<div>Edit Metadata</div><div>Edit Tags</div><div>Edit Labels</div><div>Change Artwork</div><div>Refresh Metadata</div><div>Analyze</div><div><a href="/{html.escape(kind)}/fix-match/{html.escape(item['id'])}">Fix Match</a></div><div>Unmatch</div><div>Play History</div><div>File Info</div><div>Delete</div>
</section>
...
<section class="panel danger"><h2>Delete From Server And NFS</h2>
<p class="warning">This permanently deletes the media file shown above from the server/NFS path and removes its CineVault watch-state rows. Type DELETE to enable the final delete.</p>
...
<body><section class="panel"><h1>Deleted</h1><p>{html.escape(deleted_path)}</p><p><a href="/{'movies' if kind == 'movie' else 'tv'}">Back to library</a></p></section></body></html>""")
    ...
    def direct_player(self, kind: str, item_id: str):
        ...
        playback_mode = (query.get("mode", [CINEVAULT_PLAYBACK_MODE or "direct"])[0] or "direct").lower()
        if playback_mode not in {"direct", "hls"}:
            playback_mode = "direct"
...
def hls_ffmpeg_command(path: Path, stream_dir: Path, playlist: Path) -> list[str]:
    ...
    use_vaapi = HLS_ENCODER in {"auto", "vaapi"} and can_use_vaapi()
    if use_vaapi:
        return common_input + [
            "-hwaccel",
            "vaapi",
